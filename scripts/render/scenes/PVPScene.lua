...
function PVPScene:showDamageAtPoint(x, y, damage, scale)
...
function PVPScene:addComponents()
    --add floor
    local scale = 1.0

    local battleBkg1 = CCSprite:create("huaguoshan_battleBkg_1.png")
    ...
    local battleBkg1 = CCSprite:create("battleBkg1.png")
    local battleBkg2 = CCSprite:create("battleBkg2.png")

    battleBkg1:setAnchorPoint(ccp(0.5, 0))
    battleBkg2:setAnchorPoint(ccp(0.5, 0))

    battleBkg1:setPosition(ccp(self.visibleSize.width/2, 0))
    battleBkg2:setPosition(ccp(self.visibleSize.width/2, 
        self.visibleSize.height-battleBkg2:getContentSize().height))

    battleBkg1:setZOrder(0) 
    battleBkg2:setZOrder(10000) 

    battleBkg1:setScale(scale)
    battleBkg2:setScale(scale)

    self.stageLayer:addChild(battleBkg1)
    self.stageLayer:addChild(battleBkg2)

    --添加酒店
    self.hotel = GUIReader:shareReader():widgetFromJsonFile("hotel.ExportJson")
    self.hotel:setAnchorPoint(ccp(0.5, 0))
    self.hotel:setZOrder(50)
    self.hotel:setPositionX(self.visibleSize.width/2)
    self.hotel:setPositionY(0)
    self.hotel:setScale(1.0)
    self.stageLayer:addChild(self.hotel)]]--

end

function PVPScene:findSpriteWithID(itemID)
    for i,v in pairs(self.fightersArray) do
...
function PVPScene:findBulletWithID(itemID)
    for i,v in pairs(self.bulletsArray) do
        if v.itemID == itemID then
            return v
        end
...
function PVPScene:pauseScene(pauseFightersArray)
    --[[self.paused = true

    for i,v in pairs(self.fightersArray) do
        if  v.itemID ~= pauseFightersArray then
            v:pause()
        else
            v:skill()
        end
